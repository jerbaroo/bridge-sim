...
def animate_traffic_top_view(
        c: Config, bridge: Bridge, traffic: Traffic, start_time: float,
        time_step: float, fem_runner: FEMRunner, response_type: ResponseType,
        traffic_name: str, bridge_scenario: BridgeScenario, save: str):
    ...
    def plot_f(time_index):
        ...
        top_view_vehicles(
            ...
            all_vehicles=all_vehicles, time=time_index * time_step + start_time)
        contour_cmap = plot_contour_deck(
            c=c, responses=traffic_responses[time_index], y=0,
            norm=response_norm)
        ...
        cbar = plt.gcf().colorbar(cm.ScalarMappable(
            norm=response_norm, cmap=contour_cmap), cax=cbar_ax)
