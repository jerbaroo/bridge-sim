...
if __name__ == "__main__":
    ...
    events_per_bridge = events.make_events(
        bridge=c.bridge, traffic_scenario=traffic_scenario,
        bridge_scenarios=bridge_scenarios, points=[point],
        response_types=[response_type], fem_runner=fem_runner, max_time=10,
        time_step=c.time_step)

    # Get events for each bridge scenario.
    events_per_bridge_scenario = events.get_events(
        traffic_scenario=traffic_scenario, bridge_scenarios=bridge_scenarios,
