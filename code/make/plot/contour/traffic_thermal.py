...
def thermal_deck_load(c: Config, axial_delta_temp: float, moment_delta_temp: float):
    ...
    damage_scenario_traffic_plot(
        ...
        titles=[f"{rt.name()} to {axial_delta_temp}C axial, {moment_delta_temp}C moment,\nthermal loading of the deck" for rt in response_types],
        saves=[
            c.get_image_path("contour-traffic", safe_str(f"thermal-deck-axial-{axial_delta_temp}-moment-{moment_delta_temp}-{rt.name()})") + ".pdf")
