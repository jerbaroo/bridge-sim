...
def make_convergence(c: Config, run: bool, plot: bool):
    ...
    path = c.get_image_path("convergence", "convergence_results")
    ...
    results = read_results(path + ".txt")
    plt.plot([r[6] for r in results])
    plt.savefig(path)
    plt.close()
...
def plot_convergence(c: Config, only: Optional[List[str]] = None):
    ...
    Loads data from
        'os.path.join(Config.root_generated_data_dir, "convergence")'.

    Loads a file 'verified.txt' with three columns, two for the loading position
    (x, z), and one containing the maximum response recorded in the simulation.

    Also loads files named 'convergence-*', as generated by 'make_convergence'.
    Note that the '*' indicates the plot label/machine name.
    ...
    """
    convergence_dir = os.path.join(c.root_generated_data_dir, "convergence")
    results = read_results(os.path.join(convergence_dir, "convergence.txt"))
