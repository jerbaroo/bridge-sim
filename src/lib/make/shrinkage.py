...
def plot_drying_shrinkage(config: Config, n: int = 1):
    ...
    plt.suptitle(f"Drying shrinkage")
...
def plot_autogenous_shrinkage(config: Config, n: int = 1):
    ...
    plt.ylabel(response_type.name() + " (mm)")
    plt.xlabel("Time (years)")
    plt.suptitle(f"Responses to autogenous shrinkage at X = {point.x} m, Z = {point.z} m")
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(config.get_image_path("verification/shrinkage", "autogenous-responses.pdf"))
