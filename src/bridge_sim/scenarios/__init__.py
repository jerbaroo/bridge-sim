...
from typing import NewType, Tuple, Callable, List, Optional

import numpy as np
from bridge_sim.model import Bridge, Config, Point, PierSettlement
...
class CrackedScenario(Scenario):
...
class PierSettlementScenario(Scenario):
...
class ThermalScenario(Scenario):
    ...
    def to_strain(self, c: Config, sim_responses: SimResponses):
        ...
        if not sim_responses.response_type.is_strain():
            raise ValueError(
...
def healthy_damage_w_crack_nodes(crack_f=transverse_crack):
    ...
    healthy_damage = HealthyScenario()
