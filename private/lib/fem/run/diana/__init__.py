...
def di_runner(c: Config):
    return FEMRunner(
        c, "Diana", build_models, run_model, parse_responses, convert_responses
    )
...
if __name__ == "__main__":
    ...
    expt_params = ExptParams(
        [
            SimParams([PointLoad(0, 87375)], [response_type]),
            SimParams([PointLoad(0.1, 87375)], [response_type]),
            SimParams([PointLoad(0.2, 87375)], [response_type]),
        ]
    )
