...
You will need [[https://opensees.berkeley.edu/][OpenSees]], you can download it [[https://opensees.berkeley.edu/OpenSees/user/download.php][here]]. If you are installing OpenSees on Linux then the top answer [[https://www.researchgate.net/post/How_to_install_opensees_in_UBUNTU][here]] may be of some help.
...
In the =bridge-sim= directory install project dependencies with =poetry install=. After installation you can find your poetry created virtual environment by using =poetry show -v=. You might need to add the path to the virtual environment manually in your IDE.

# If you have an error and are on the Windows OS you may need to =poetry install pypiwin32=. I do not run Windows so can be of limited help there.
