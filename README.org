...
#+BEGIN_SRC bash
docker run -ite LC_ALL=C.UTF-8 -e LANG=C.UTF-8 barischrooneyj/bridge-sim:v0.0.3
#+END_SRC
...
d297a50ff165        barischrooneyj/bridge-sim:v0.0.3   "/bin/bash"      37 seconds ago      Up 37 seconds               vigorous_leavitt
$ docker cp d297a50ff165:/bridge-sim/docker.png docker.png
#+END_SRC

To copy other files to the Docker container you can run =docker cp local-files/ d297a50ff165:/bridge-sim/=. Run =docker cp --help= or search the web for more information.
