...
#+BEGIN_SRC bash
$ docker run -it barischrooneyj/bridge-sim:v0.0.8.6
#+END_SRC
...
d297a50ff165        barischrooneyj/bridge-sim:v0.0.8.6   "/bin/bash"      37 seconds ago      Up 37 seconds               vigorous_leavitt
...
Once you have SSHed into the Lisa cluster you can build a Singularity image of =bridge_sim= on Lisa with =singularity pull docker://barischrooneyj/bridge-sim:v0.0.8.6=. If the =pull= command fails or the generated =.sif= file is corrupted then try deleting the =.sif= file and run the =pull= command again.
